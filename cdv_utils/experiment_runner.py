...
def run_single_seed_synthetic_experiment(alpha, exp_seed, n_train, w_cols, top_variants, k,
    ...
    best_global_model = None
    if global_estimator_scores:
        valid_global_models = {k_name: v for k_name, v in global_estimator_scores.items() if v['r2'] > r2_threshold}
...
def run_multi_seed_synthetic_experiment(alpha, experiment_seeds, n_train, w_cols, top_variants, k,
                                        training_variant_patterns, test_variant_dataframes,
                                        val_variant_dataframes, global_test_variant_dataframes,
                                        global_val_variant_dataframes, results_save_path,
                                        initial_seed=420, r2_threshold=0.2):
    """
    Run the complete multi-seed synthetic experiment for a given alpha.
    ...
    print("=" * 70)
    print(f"Starting Synthetic Multi-Seed Experiment: alpha={alpha}, {len(experiment_seeds)} seeds")
    print("=" * 70)
    
    for seed_idx, exp_seed in enumerate(tqdm(experiment_seeds, desc=f"alpha={alpha}")):
        print(f"\n{'=' * 70}")
        print(f"SEED {exp_seed} ({seed_idx + 1}/{len(experiment_seeds)}) | alpha={alpha}")
        print(f"{'=' * 70}")
        
        try:
            seed_results = run_single_seed_synthetic_experiment(
                alpha=alpha, exp_seed=exp_seed, n_train=n_train,
            ...
            print(f"[{exp_seed}] ✓ Completed. Results saved.")
    ...
    print("\n" + "=" * 70)
    print(f"Synthetic Experiment Complete! alpha={alpha}")
    print("=" * 70)
    print(f"Total seeds processed: {len(results_by_seed)}")
    print(f"Results saved to: {results_save_path}")
